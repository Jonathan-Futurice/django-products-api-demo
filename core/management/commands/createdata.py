...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for _ in range(15):
            ...
            image_path = default_storage.save(f'images/{fake.unique.pystr()}.jpeg', ContentFile(image_data))
            barcode = fake.ean(length=13)
            description = fake.paragraphs(nb=1)

            print(f'{name}, {barcode}, {description}, {image_path}')

            Product.objects.create(name=name, description=description[0], barcode=barcode, image_path=image_path)
